...
The Payments section records salary payments, INPS contributions, thirteenth-month salary, TFR, expense reimbursements and other payments. Each record can be Pending or Paid and can include payment date, reference period, description, notes and supporting attachments. Amounts still due are visually highlighted in the UI. Every payment marked **Paid** provides a professional downloadable PDF receipt showing the amount due, the amount actually paid and the residual; when an employer signature is stored, it is inserted automatically in the receipt. INPS contribution payments are available only for workers with an INPS position recorded. When a payment is created or changed for a month that already has an archived monthly payroll, that payroll is regenerated with the current payment amounts and replaces the file in the same archive record; the same refresh occurs when a payment is moved back to Pending or deleted.
