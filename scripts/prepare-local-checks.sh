...
set -euo pipefail
cd "$(dirname "$0")/.."
echo "== colf-manager hardening full package r25: environment preparation =="

required=(VERSION pyproject.toml README.md src/colf_manager/templates/dashboard.html)
for path in "${required[@]}"; do
  if [[ ! -e "$path" ]]; then
    echo "ERROR: missing $path; use the complete r25 package." >&2
    exit 2
  fi
done
...
PY
echo
echo "Dependencies installed for full package r25."
